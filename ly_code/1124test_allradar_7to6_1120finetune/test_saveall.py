...
class PreDiffSEVIRPLModule(LatentDiffusion):
    ...
    def __init__(self,
        ...
        # self.test_example_data_idx_list = list(range(
        #     oc.eval.test_example_data_idx_start,
        #     oc.eval.test_example_data_idx_end + 1
        # ))
    ...
    def  get_sevir_datamodule(dataset_cfg,
                             ...
                             num_workers: int = 8):
        # 从 dataset_cfg 中读取可选字段（兼容旧 cfg）
        npy_dir = dataset_cfg.get("npy_dir") if isinstance(dataset_cfg, dict) else None
        npy_num_samples = dataset_cfg.get("npy_num_samples") if isinstance(dataset_cfg, dict) else None
        random_split = dataset_cfg.get("random_split", False) if isinstance(dataset_cfg, dict) else False
        split_seed = dataset_cfg.get("split_seed", 0) if isinstance(dataset_cfg, dict) else 0
        # split_ratios = dataset_cfg.get("split_ratios", [0.7, 0.15, 0.15])  # ✅ 新增
        export_split_csv = dataset_cfg.get("export_split_csv", None)       # ✅ 新增
        ...
        dm = SEVIRLightningDataModule(
            ...
            # 新增可选参数，传给 DataModule
            npy_dir=npy_dir,
            npy_num_samples=npy_num_samples,
            random_split=random_split,
            split_seed=split_seed,)
            # split_ratios=split_ratios,              # ✅ 新增
            # export_split_csv=export_split_csv )      # ✅ 新增   
        return dm
    ...
    def in_slice(self):
        if not hasattr(self, "_in_slice"):
            ...
            # print(f"in_slice: {self._in_slice}, out_slice: {out_slice}")
    ...
    def _get_input_sevirlr(self, batch, return_verbose=False):
        #======================临时测试：强制使得inslice和outslice固定======================
        # in_slice  = [slice(None), slice(None, 8), slice(None), slice(None), slice(None)]
        # out_slice = [slice(None), slice(8, 15), slice(None), slice(None), slice(None)]
        # seq = batch
        # print(f"_get_input_sevirlr: batch.shape = {batch.shape}")
        # in_seq = seq[in_slice]
        # print(f"_get_input_sevirlr: in_seq.shape = {in_seq.shape}")
        # out_seq = seq[out_slice].contiguous()
        # print(f"_get_input_sevirlr: out_seq.shape = {out_seq.shape}")
...
def get_parser():
    ...
    parser.add_argument('--save', default='1124_allradar_test_7to6_699ckpt_1120finetune', type=str)
    parser.add_argument('--nodes', default=1, type=int,
                        help="Number of nodes in DDP training.")
    parser.add_argument('--gpus', default=1, type=int,
                        help="Number of GPUS per node in DDP training.")
    parser.add_argument('--cfg', default='/home/user01/personal_file/ly/PreDiff-25fall/ly_code/1124test_allradar_7to6_1120finetune/prediff_sevirlr_v1_finetune.yaml', type=str)
    ...
    # 新增用于直接可视化 dataloader 的开关与参数
    parser.add_argument('--visualize', default=False, action='store_true',
                        help='Visualize samples from train/val/test dataloaders and exit.')
    parser.add_argument('--visualize-batches', default=1, type=int,
                        help='Number of batches to visualize per loader.')
    # 使用验证集作为测试输入（替代 test set）
    parser.add_argument('--use-val-for-test', default=True, action='store_true',
                        help='Run test using validation dataloader instead of test dataloader.')
...
def main():
    ...
    # 可视化 dataloader 样本并退出（可选）
    if args.visualize:
        print("[INFO] Visualizing dataloaders and exiting...")
        vis_dir = os.path.join(pl_module.save_dir, "dataloader_vis")
        os.makedirs(vis_dir, exist_ok=True)
        
        # ====================251023可视化时train loader设置shuffle=False且不旋转增强====================
        # 手动构造 dataloader，便于外部设置 shuffle 与临时禁用 augmentation（用于可视化）
        # 注意：这里只修改用于可视化的 loader，不会更改后续 trainer.fit 使用的 dm.train_dataloader()
        # 禁用增强（不旋转、不翻转）
        if hasattr(dm, "sevir_train"):
            dm.sevir_train.aug = lambda x: x
        if hasattr(dm, "sevir_val"):
            dm.sevir_val.aug = lambda x: x
        if hasattr(dm, "sevir_test"):
            dm.sevir_test.aug = lambda x: x

        loaders = {
            # 外部把 train loader 设置为 shuffle=True
            "train": DataLoader(dm.sevir_train, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers),
            "val": DataLoader(dm.sevir_val, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers),
            "test": DataLoader(dm.sevir_test, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers),
        }

        # ====================251023可视化时默认加载train loader设置(shuffle=True且旋转增强)====================
        # loaders = {
        ...
        for name, loader in loaders.items():
            for batch_idx, batch in enumerate(loader):
                if batch_idx >= args.visualize_batches:
                    break
                # 使用 pl_module.get_input 获取 context 和 target（保持与训练时一致的切片方式）
                try:
                    target_seq, cond, context_seq = pl_module.get_input(batch, return_verbose=True)
                except TypeError:
                    # 兼容若 get_input 返回(out_seq, cond) 的情况
                    target_seq, cond = pl_module.get_input(batch, return_verbose=False)
                    context_seq = cond["y"]
                # 取第一个样本进行可视化
                context = context_seq[0].detach().float().cpu().numpy()
                target = target_seq[0].detach().float().cpu().numpy()
                png_name = f"{name}_batch{batch_idx}.png"
                vis_sevir_seq(
                    save_path=os.path.join(vis_dir, png_name),
                    seq=[context, target],
                    label=["context", "target"],
                    interval_real_time=dataset_cfg.get("interval_real_time", 10),
                    plot_stride=dataset_cfg.get("plot_stride", 1),
                    fs=pl_module.oc.eval.font_size,
                    label_offset=pl_module.oc.eval.label_offset,
                    label_avg_int=pl_module.oc.eval.label_avg_int,
                )
                # # 可视化该 batch 中的所有样本（按 batch 内索引分别保存）
                # batch_vis = context_seq.shape[0]
                # for si in range(batch_vis):
                #     context = context_seq[si].detach().float().cpu().numpy()
                #     target = target_seq[si].detach().float().cpu().numpy()
                #     png_name = f"{name}_batch{batch_idx}_sample{si}.png"
                #     vis_sevir_seq(
                #         save_path=os.path.join(vis_dir, png_name),
                #         seq=[context, target],
                #         label=[f"context_{si}", f"target_{si}"],
                #         interval_real_time=dataset_cfg.get("interval_real_time", 10),
                #         plot_stride=dataset_cfg.get("plot_stride", 1),
                #         fs=pl_module.oc.eval.font_size,
                #         label_offset=pl_module.oc.eval.label_offset,
                #         label_avg_int=pl_module.oc.eval.label_avg_int,
                #     )
            print(f"[INFO] Saved {name} visualization to {vis_dir}")
        return
    
    # 在 dm.setup() 后立刻移动或另存一份 CSV
    original_record_path = os.path.join(dm.sevir_dir, "split_record.csv")
    new_record_path = os.path.join(pl_module.save_dir, "split_record.csv")
    
    if os.path.exists(original_record_path):
        import shutil
        shutil.copy(original_record_path, new_record_path)
        print(f"[INFO] Copied split record to {new_record_path}")
    ...
    #=======训练逻辑 1.pretrained=True则加载预训练模型进行测试 2.test=True则加载指定模型进行测试 3.否则进行训练========
    if args.pretrained:
        ...
        if args.ckpt_name is not None:
            ...
            # ckpt_path = '/data/25fall_nowcasting/ly/PreDiff-25fall/experiments-25fall/1105_allradar_ep10000_val50_finetune/model/checkpoints/149.ckpt'
        ...
        #              datamodule=dm, )
        if args.use_val_for_test:
            trainer.test(model=pl_module, dataloaders=test_dataloaders)
        else:
        ...
        # # 2️⃣ 直接重写 dataloader 方法（覆盖 DataModule 默认行为）
        # dm.train_dataloader = lambda: DataLoader(
        #     dm.sevir_train, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers
        # )
        # dm.val_dataloader = lambda: DataLoader(
        #     dm.sevir_val, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers
        # )
        # dm.test_dataloader = lambda: DataLoader(
        #     dm.sevir_test, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers
        # )    
        ...
        # # ==================== 251023 可视化时 train loader 设置 shuffle=False 且不旋转增强 ====================
        # from torch.utils.data import DataLoader
        ...
        # # 手动 dataloader（不 shuffle，方便单样本测试）
        # loaders = {
        #     "train": DataLoader(dm.sevir_train, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers),
        #     "val": DataLoader(dm.sevir_val, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers),
        #     "test": DataLoader(dm.sevir_test, batch_size=dm.batch_size, shuffle=False, num_workers=dm.num_workers),
        ...
        # # ⚠️ 关键修改：trainer.fit 用自定义的 dataloader，而不是 datamodule
        # trainer.fit(
        #     model=pl_module,
