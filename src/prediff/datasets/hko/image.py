# Code is adapted from https://github.com/sxjscience/HKO-7/blob/master/nowcasting/image.py
...
def quick_read_frames(
        path_list,
        im_w=None, im_h=None,
        resize=False, frame_size=None,
        grayscale=True,
        layout="NCHW"):
    ...
    """
    if grayscale:
        einops_orig_layout = "N H W"
        einops_out_layout = " ".join(layout.replace("C", "1"))
    else:
        einops_orig_layout = "N H W C"
        einops_out_layout = " ".join(layout)
    ...
    if resize:
        ...
        if not grayscale:
            resize_storage = resize_storage[..., ::-1]
        return rearrange(resize_storage, f"{einops_orig_layout} -> {einops_out_layout}")
        ...
        if not grayscale:
            read_storage = read_storage[..., ::-1]
        return rearrange(read_storage, f"{einops_orig_layout} -> {einops_out_layout}")
