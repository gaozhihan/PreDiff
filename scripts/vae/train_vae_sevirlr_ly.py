...
class VAESEVIRPLModule(pl.LightningModule):
    ...
    def get_input(self, batch):
        target_bchw = rearrange(batch, "b t h w c -> (b t) c h w").contiguous()
        mask = None
    ...
    def save_vis_step_end(
        ...
        #====================1111新增判断可视化的pred和target是否一致====================
        print(f"[DEBUG] target shape: {target.shape}, pred shape: {pred.shape}")
        print(f"[DEBUG] target range: [{target.min():.3f}, {target.max():.3f}]")
        print(f"[DEBUG] pred range: [{pred.min():.3f}, {pred.max():.3f}]")
        print(f"[DEBUG] MSE between target and pred: {np.mean((target - pred)**2):.6f}")
        
        if np.allclose(target, pred):
            print("[WARNING] Target and pred are identical!")
...
def get_parser():
    ...
    parser.add_argument('--save', default='1128_vae_1sevir_pretrained', type=str)
    parser.add_argument('--gpus', default=1, type=int)
    parser.add_argument('--cfg', default='/home/user01/personal_file/ly/PreDiff-25fall/scripts/vae/cfg.yaml', type=str)
...
def main():
    ...
    print("\n================ CHECKPOINT DEBUG ================")
    print("[DEBUG] lightning version:", pl.__version__)
    for cb in trainer.callbacks:
        if isinstance(cb, ModelCheckpoint):
            print("  - ModelCheckpoint:")
            print("      monitor      =", cb.monitor)
            print("      dirpath      =", cb.dirpath)
            print("      filename     =", cb.filename)
            print("      save_top_k   =", cb.save_top_k)
            print("      mode         =", cb.mode)
            print("      best_model   =", cb.best_model_path)
            print("      last_model   =", cb.last_model_path)
    print("=================================================\n")
    ...
    if args.pretrained:
        ...
        print(f"[DEBUG] Loading VAE from: {vae_ckpt_path}")
        print(f"[DEBUG] File exists: {os.path.exists(vae_ckpt_path)}")

        state_dict = torch.load(vae_ckpt_path,
                                map_location=torch.device("cpu"))
        print(f"[DEBUG] State dict keys: {list(state_dict.keys())[:5]}...")  # 打印前5个key
         # 加载前检查模型参数
        original_params = {name: param.clone() for name, param in pl_module.torch_nn_module.named_parameters()}
        pl_module.torch_nn_module.load_state_dict(state_dict=state_dict)
        
        # 检查参数是否真的改变了
        for name, param in pl_module.torch_nn_module.named_parameters():
            if torch.allclose(original_params[name], param):
                print(f"[WARNING] Parameter {name} did not change after loading!")
                
        trainer.test(model=pl_module,
                     datamodule=dm)
        
        # #=======================1111新增VAE有效性验证代码（是否调用了潜空间）=======================
        # print("\n=== VAE有效性验证 ===")
        ...
        # pl_module.eval()
        # with torch.no_grad():
        #     target_bchw, _ = pl_module.get_input(batch=test_batch)
        #     posterior = pl_module.torch_nn_module.encode(target_bchw)
        #     z = posterior.sample()
        #     print(f"潜在空间统计: mean={z.mean():.4f}, std={z.std():.4f}")
            
        #     # 测试生成能力 - 修正这部分
        #     try:
        #         # 方法1: 直接使用解码器
        #         random_z = torch.randn_like(z)
        #         generated = pl_module.torch_nn_module.decode(random_z)
        #         print(f"生成图像范围: [{generated.min():.3f}, {generated.max():.3f}]")
        ...
        #         try:
        #             # 创建一个假的输入，只为了获取解码器输出
        #             dummy_input = torch.randn_like(target_bchw)
        #             generated, _ = pl_module.torch_nn_module(dummy_input, sample_posterior=False)
        #             print(f"通过forward生成图像范围: [{generated.min():.3f}, {generated.max():.3f}]")
        #         except Exception as e2:
        #             print(f"forward也失败: {e2}")
