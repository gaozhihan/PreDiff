...
from prediff.diffusion.knowledge_alignment.hko import HKOAvgIntensityAlignment


pytorch_state_dict_name = "hko_earthformerunet.pt"
...
class PreDiffHKOPLModule(LatentDiffusion):
    ...
    def __init__(self,
        ...
        super(PreDiffHKOPLModule, self).__init__(
        ...
        if self.use_alignment:
            ...
            self.alignment_obj = HKOAvgIntensityAlignment(
        ...
        if self.oc.eval.eval_unaligned:
            ...
            self.valid_score = HKOSkillScore(
        ...
        if self.oc.eval.eval_aligned:
            ...
            self.valid_aligned_score = HKOSkillScore(
                mode=self.oc.dataset.metrics_mode,
                seq_len=self.oc.layout.out_len,
                layout=self.layout,
                threshold_list=self.oc.dataset.threshold_list,
            ...
            self.test_aligned_score = HKOSkillScore(
                mode=self.oc.dataset.metrics_mode,
                seq_len=self.oc.layout.out_len,
                layout=self.layout,
                threshold_list=self.oc.dataset.threshold_list,
    ...
    def get_dataset_config(cls):
        ...
        cfg.val_ratio = 0.1
        cfg.train_pd_path = None
        cfg.val_pd_path = None
        cfg.test_pd_path = None
        cfg.seq_len = 25
        cfg.max_consecutive_missing = 2
        cfg.stride = 5
        cfg.height = 480
        cfg.width = 480
        cfg.base_freq = "6min"
        cfg.downscaling_scale = [3, 3]
        cfg.interpolate_resize = [128, 128]
        cfg.norm_mode = "01"
        cfg.aug_mode = "1"
        cfg.seed = 0
        cfg.weighted_sampler = "0"
        cfg.metrics_mode = "0"
    ...
    def get_input(self, batch, **kwargs):
        ...
        """
        return self._get_input_hko(batch=batch, return_verbose=kwargs.get("return_verbose", False))

    @torch.no_grad()
    def _get_input_hko(self, batch, return_verbose=False):
        seq, mask = batch  # NTHW
        # seq = seq * mask
        seq = seq.unsqueeze(-1)  # NTHWC
        in_seq = seq[self.in_slice]
        out_seq = seq[self.out_slice].contiguous()
        if return_verbose:
            mask = mask.unsqueeze(-1)  # NTHWC
            return out_seq, {"y": in_seq}, in_seq, mask
        else:
...
def get_parser():
    ...
    parser.add_argument('--save', default='tmp_hko', type=str)
    ...
    parser.add_argument('--ckpt_name', default=None, type=str,
                        help='The model checkpoint trained on HKO.')
...
def main():
    ...
    if args.pretrained:
        args.cfg = os.path.abspath(os.path.join(os.path.dirname(__file__), "prediff_hko_v1.yaml"))
        # Download pretrained weights
        download_pretrained_weights(ckpt_name=pretrained_hko_vae_name,
                                    save_dir=default_pretrained_vae_dir,
                                    exist_ok=False)
        download_pretrained_weights(ckpt_name=pretrained_hko_earthformerunet_name,
                                    save_dir=default_pretrained_earthformerunet_dir,
                                    exist_ok=False)
        download_pretrained_weights(ckpt_name=pretrained_hko_alignment_name,
    ...
    if args.cfg is not None:
        ...
        dataset_cfg = OmegaConf.to_object(PreDiffHKOPLModule.get_dataset_config())
        micro_batch_size = 1
    ...
    dm = PreDiffHKOPLModule.get_hko_datamodule(
    ...
    total_num_steps = PreDiffHKOPLModule.get_total_num_steps(
        epoch=max_epochs,
        num_samples=dm.num_train_samples,
        total_batch_size=total_batch_size,
    )
    pl_module = PreDiffHKOPLModule(
