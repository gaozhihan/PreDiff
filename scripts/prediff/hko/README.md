...
Run the following command to train PreDiff on HKO-7 dataset.
```bash
cd ROOT_DIR/PreDiff
MASTER_ADDR=localhost MASTER_PORT=10001 python ./scripts/prediff/hko/train_hko_prediff.py --gpus 2 --cfg ./scripts/prediff/hko/cfg.yaml --save tmp_hko_prediff
```
Or run the following command to directly load pretrained checkpoint for test.
```bash
cd ROOT_DIR/PreDiff
MASTER_ADDR=localhost MASTER_PORT=10001 python ./scripts/prediff/hko/train_hko_prediff.py --gpus 2 --pretrained --save tmp_hko_prediff
```
Run the following command to train PreDiff using multi-node DDP.
```bash
# On the master node
MASTER_ADDR=localhost MASTER_PORT=10001 WORLD_SIZE=16 NODE_RANK=0 python ./scripts/prediff/hko/train_hko_prediff.py --nodes 2 --gpus 2 --cfg ./scripts/prediff/hko/train_hko_prediff/cfg.yaml --save tmp_hko_prediff
# On the 1st node
MASTER_ADDR=$master_ip MASTER_PORT=10001 WORLD_SIZE=16 NODE_RANK=1 python ./scripts/prediff/hko/train_hko_prediff.py --nodes 2 --gpus 2 --cfg ./scripts/prediff/hko/train_hko_prediff/cfg.yaml --save tmp_hko_prediff
```
Run the tensorboard command to visualize the experiment records
```bash
cd ROOT_DIR/PreDiff
tensorboard --logdir ./experiments/tmp_hko_prediff/lightning_logs
```
